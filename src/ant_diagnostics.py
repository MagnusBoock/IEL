...
class Visualizer:
    def __init__(self, env_name, viz_env, dataset, discount):
        data_path = osp.abspath(osp.join(osp.dirname(
            __file__), f'../antmaze_aux/{env_name}-aux.npz'))
    ...
    def get_gradients(self, policy_fn, N=20):
        ...
        base_observations = np.tile(
            base_observation, (observations.shape[0], 1))
        base_observations[:, :2] = observations

        policies = policy_fn(base_observations)
        directions = policies / \
            (1e-6 + np.linalg.norm(policies, axis=1, keepdims=True))
        return directions
    ...
    def get_distances(self, trajs):
        ...
        final_points = np.stack(
            [final_points[:, 1], final_points[:, 0]], axis=1)
